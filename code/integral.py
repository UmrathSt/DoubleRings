...
def a_correction(kR, l1, m1, lmax):
    if (l1+m1)%2 or m1 == 0:
        return 0
    denom = (Nlm(l1, m1)*Plm(l1, m1)/kR*m1)**2*hdl(l1,kR)*jdl(l1,kR)
    factor = 1
    ls = np.arange(max(1,abs(m1)), lmax+1)
    factor += np.array([(Nlm(l, m1)*diffPlm(l, m1))**2*hl(l, kR)*jl(l,kR) for l in ls]).sum()
    factor += np.array([(Nlm(l, m1)*Plm(l, m1)/kR*m1)**2*hdl(l, kR)*jdl(l,kR) for l in ls]).sum()
...
if __name__ == "__main__":
    ...
    for L in l:
        result_a[L-1] = a_correction(kr, L, mTM, LMAX) 
        result_b[L-1] = b_correction(kr, L, mTE, LMAX) 
